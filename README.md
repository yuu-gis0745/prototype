...
測量・森林GISコンサルタント会社にて、測量業務では、調査後の測量データを指定のExcel様式へ手入力する作業があり、転記ミスや誤判読の発生、入力後の検査に時間を要する点を課題として感じていました。
...
現職での実務課題をそのまま題材にすることで、机上のサンプルアプリではなく実業務に即した課題設定・要件定義ができた点も、本プロジェクトの特徴です。
...
森林調査票(xlsx)をアップロードすると、以下のようなJSONが返ります。

リクエスト（`/docs`の場合はファイル選択フォームから、curlの場合は以下のように送信）：

```bash
curl -X POST "http://127.0.0.1:8000/analyze" \
  -F "survey_file=@sample_forest_survey_3plots_with_error.xlsx"
```
  ...
  "error_log": [
    {
      ...
      "check_item": "調査ID未入力",
      "severity": "エラー",
      "error_message": "調査IDが未入力です",
      "fix_action": "調査IDを入力してください"
    }
  ],
  "gis_summary": [],
  "survey_text_for_llm": "..."
...
レスポンス例（入力エラーがない場合、GISサマリーが生成される）：
...
`error_log`はそのままcsv形式のエラーログに、`gis_summary`はQGIS結合用CSVに変換されます。
...
- pytest等を用いた自動テストの導入（現時点では、サンプルファイルを用いた手動での動作確認のみで、単体テスト・結合テストは未実装です）
...
### 16-2．連絡先・その他のポートフォリオ

- GitHub: [@yuu-gis0745](https://github.com/yuu-gis0745)
